...
## Create bar plots of means + individual jitter points + standard errors
## https://www.datanovia.com/en/lessons/ggplot-error-bars/
## Note: a potentially easier way using ggpubr (untested) can be found here:
...
Fig423b.tallna.annotated <- Fig423b.tallna %>%
  ggplot(aes(Age, HSC)) +
  geom_col(data = Fig423b.tallna.summary, fill = c("#56B4E9", "white", "gray"), color = "black", width = 0.7) +
  scale_x_discrete(limits = c("Young", "Old", "Old asplenic")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100)) +
